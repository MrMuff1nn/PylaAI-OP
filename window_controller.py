...
class WindowController:
    def __init__(self):
        ...
        try:
            ...
            # If the user picked a specific emulator but we haven't found a device
            # that matches its port, probe its ports with a fast TCP check and
            # only call adb.connect on ones that are actually listening — a blind
            # adb.connect to a closed port takes ~1s, which compounds across 30
            # candidates. Note: 5037 (ADB server) is filtered out by _unique_ports.
            if not preferred_devices and selected_emulator != "Others":
